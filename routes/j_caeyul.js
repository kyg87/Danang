...
var db = mongojs('mongodb://dridy:fkawk1@ds121906.mlab.com:21906/danang',['j.caeyul']);

router.get('/j_caeyul', function(req, res, next){
    ...
    db.j.caeyul.count( function(err,totalCount){
        ...
        db.j.caeyul.find().sort({ '_id': -1 }).skip((parseInt(req.query.page) - 1) * req.query.size).limit(parseInt(req.query.size)).toArray(function(err, result) {
            if (err) throw err;
